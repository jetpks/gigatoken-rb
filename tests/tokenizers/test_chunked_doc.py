...
def test_whole_doc_parallel_matches_serial(jeton_tok, big_text, tmp_path):
    path = tmp_path / "doc.txt"
    path.write_text(big_text)
    parallel = jeton_tok.encode_files([str(path)])  # whole file = one doc
    serial = jeton_tok.encode(big_text)
    assert len(parallel) == 1
    assert parallel[0].tolist() == serial.tolist()


def test_single_doc_batch_matches_serial(jeton_tok, big_text):
    out = jeton_tok.encode_batch([big_text])
    assert len(out) == 1
    assert out[0].tolist() == jeton_tok.encode(big_text).tolist()
